...
int main(int argc, char *argv[])
{
  ...
  if(view) viewBitmap(buffer,width,height, trueColor);
  //free(buffer);
  return EXIT_SUCCESS;
}//}}}
...
void viewBitmap(BMPColor** buffer, int width, int height, char trueColor){
  ...
  printf("   | ");
  for(int x=0;x<width;x++) printf("%x", x % 16);
  ...
  for(int y=0;y<height;y++)
  {
    ...
    for(int x=0;x<width;x++)
    {
      ...
      if(trueColor)printf("\x1b[48;2;%d;%d;%dm \x1b[0m", currentPixel.r, currentPixel.g, currentPixel.b);
      else printf("\e[48;5;%dm \e[0m",rgbToAnsi256(currentPixel.r, currentPixel.g, currentPixel.b));
    }
    printf("\n");
  }
}
...
void testImage()
{
  ...
  for(int i=0;i<sizeof(stars)/sizeof(Star); i++)
    for(int y=0;y<height;y++)
      for(int x=0;x<width;x++)
      {
        p.x = x;
